...
const getIconForSensor = (id: string) => {
  switch (id) {
    case 'INT-001': return Gauge;      // Pressure
    case 'INT-002': return Droplets;   // Level
    case 'INT-003': return Activity;   // Flow
...
const IntakeSensorCard: React.FC<IntakeSensorCardProps> = ({ tag, index }) => {
  ...
  // For Level (LT) sensor, show "No Data" only when value is exactly 0
  // For other sensors, "No Data" is based on connection timeout
  const shouldShowNoData = () => {
    if (tag.id === 'INT-002') { // Level (LT) sensor
      return tag.value === 0;
    }
    return connectionStatus === 'no-data';
  };
  ...
  const getConnectionBadge = () => {
    ...
    if (tag.id === 'INT-002') {
      if (tag.status !== 'disconnected') {
        ...
        return (
          <span className="status-indicator bg-destructive/10 text-destructive border-destructive/30">
...
export default IntakeSensorCard;
